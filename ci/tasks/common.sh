#
# This file is borrowed from Concourse's `docker-image-resource`
# Source: https://github.com/concourse/docker-image-resource/blob/master/assets/common.sh
# License: https://github.com/concourse/docker-image-resource/blob/master/LICENSE
#
...
start_docker() {
    local download_conn=${1:-3}
    local upload_conn=${2:-3}
    local registries=${3:-""}
    local mirror=${4:-""}
    ...
    server_args="${server_args} " \
        "--max-concurrent-downloads=${download_conn} " \
        "--max-concurrent-uploads=${upload_conn}"
